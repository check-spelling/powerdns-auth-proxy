...
def client():
    ...
    if not schema_paths:
        raise Exception(
            "Unsupported OS. Cannot find example sqlite schema. Looked in: "
            + ":".join(ALL_SCHEMA_PATHS)
        )
    ...
    with closing(sqlite3.connect(pdns_db_path)) as db:
        ...
        db.execute(
            "INSERT INTO domains (name, type, account) VALUES ('example.net', 'MASTER', 'nobody');"
        )  # create a domain that the demo user can't read later
        db.commit()
        ...
        try:
            if (
                requests.get(
                    ...
                    timeout=1,
                    headers={"X-API-Key": "7128ae9eb680a14390ee22a988a9d01a"},
                ).status_code
                == 200
            ):
                break
...
def test_api_zone_list(client):
    ...
    response = client.get(
        "/api/v1/servers/localhost/zones", headers=api_key_header(client)
    )
    assert response.status_code < 400
...
def test_api_zone_delete(client):
    ...
    response = client.delete(
        "/api/v1/servers/localhost/zones/example.org.", headers=api_key_header(client)
    )
    assert response.status_code < 400

    # delete a zone belonging to another user, should return exactly 403 to prevent enumeration
    response = client.delete(
        "/api/v1/servers/localhost/zones/example.net.", headers=api_key_header(client)
    )
    assert response.status_code == 403

    # delete a zone that doesn't exist, should return exactly 403 to prevent enumeration
    response = client.delete(
        "/api/v1/servers/localhost/zones/example.com.", headers=api_key_header(client)
    )
    assert response.status_code == 403
...
def test_api_zone_put(client):
    ...
    response = client.put(
        "/api/v1/servers/localhost/zones/example.net.",
        headers=api_key_header(client),
        json=json,
    )
    assert response.status_code == 403
...
def test_api_zone_notify(client):
    ...
    response = client.put(
        "/api/v1/servers/localhost/zones/example.net./notify",
        headers=api_key_header(client),
    )
    assert response.status_code == 403
    ...
    response = client.put(
        "/api/v1/servers/localhost/zones/example.org./notify",
        headers=api_key_header(client),
    )
    assert response.status_code < 400
